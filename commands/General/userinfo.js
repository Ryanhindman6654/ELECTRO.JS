...
class Userinfo extends Command {
    ...
    async run (message, args, data) {
        ...
        if(member){
            ...
            .addField(message.language.get("USERINFO_FIELDS")[10], (member.roles.size > 10 ? member.roles.cache.map((r) => r).slice(0, 9).join(", ")+message.language.get("USERINFO_MORE_ROLES", member.roles.size - 10) : (member.roles.size < 1) ? message.language.get("USERINFO_NO_ROLE") : member.roles.cache.map((r) => r).join(", ")));
        }
